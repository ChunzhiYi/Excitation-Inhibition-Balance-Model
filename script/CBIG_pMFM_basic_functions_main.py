...
def CBIG_mfm_multi_simulation(parameter, sc_mat, t_epochlong, n_dup):
	...
	start = time.time()
	for i in range(1000):
		d_y, dS_I = CBIG_mfm_rfMRI_ode_inhibition(y_t, S_I, parameter, sc_mat)
...
def CBIG_mfm_multi_simulation_output(parameter, sc_mat, t_epochlong, n_dup, perturb, perturb1):
	'''
	Function used to generate the simulated BOLD signal using mean field
	model and hemodynamic model
	Each parameter set is ussed to simulated multiple times to get
	...
	count = 0
	y_bold = torch.zeros((n_nodes, n_set, int(n_samples / (t_bold / d_t) + 1)))
	y_S_I = torch.zeros((n_nodes, n_set, int(n_samples  / (t_bold / d_t)+ 1)))
	y_S_E = torch.zeros((n_nodes, n_set, int(n_samples / (t_bold / d_t) + 1)))
	# Warm up
	start = time.time()
	for i in range(1000):
		d_y, dS_I = CBIG_mfm_rfMRI_ode_inhibition(y_t, S_I, parameter, sc_mat)
	...
	for i in range(n_samples):
		d_y, dS_I = CBIG_mfm_rfMRI_ode_inhibition(y_t, S_I, parameter, sc_mat)
		if torch.isnan(d_y).any():
			print('222')
		if torch.isnan(dS_I).any():
			print('223')
		noise_level = d_w[:, :, i + 1000].repeat(1, 1,
												 n_num).contiguous().view(
													 -1, n_nodes)
		...
		y_S_I[13, :, :] = y_S_I[13, :, :] + perturb

		y_S_E[48, :, :] = y_S_E[48, :, :] + perturb1
		...
		y_bold_temp = 100 / p_costant * v_0 * (
			k_1 * (1 - q_t) + k_2 * (1 - q_t / v_t) + k_3 * (1 - v_t))
		if torch.isnan(y_bold_temp).any():
			print('111')
		y_bold[:, :, count] = y_bold_temp[:, :, 0]
		count = count + ((i + 1) % (t_bold / d_t) == 0) * 1
	elapsed = time.time() - start
	...
	cut_index = int(t_pre / t_bold)
	bold_d = y_bold[:, :, cut_index + 1:y_bold.shape[2]]
	S_E_d = y_S_I[:, :, cut_index + 1:y_S_I.shape[2]]
	S_I_d = y_S_E[:, :, cut_index + 1:y_S_E.shape[2]]

	return bold_d, S_E_d, S_I_d

def CBIG_mfm_multi_simulation_5HT(parameter, sc_mat, t_epochlong, n_dup):
	'''
	Function used to generate the simulated BOLD signal using mean field
	model and hemodynamic model
	Each parameter set is ussed to simulated multiple times to get
	...
	# Initializing neurotransmitter system
	Mn_5HT = torch.zeros((n_nodes, n_set))
	Sn_5HT = torch.zeros((n_nodes, n_set))
	dMn_5HT = torch.zeros((n_nodes, n_set))
	dSn_5HT = torch.zeros((n_nodes, n_set))
	Mn_5HT[:, :] = 0.001
	Sn_5HT[:, :] = 0.001
	...
	start = time.time()
	for i in range(1000):
		d_y, dS_I, dMn_5HT, dSn_5HT = CBIG_mfm_rfMRI_ode_inhibition_5HT(y_t, S_I, parameter, sc_mat, Mn_5HT, Sn_5HT)
		noise_level = d_w[:, :, i].repeat(1, 1, n_num).contiguous().view(
			-1, n_nodes)
		y_t = y_t + d_y * d_t + w_coef * torch.transpose(noise_level, 0, 1)
		S_I = S_I + dS_I * d_t + w_coef * torch.transpose(noise_level, 0, 1)
		Mn_5HT = Mn_5HT + dMn_5HT * d_t
		Sn_5HT = Sn_5HT + dSn_5HT * d_t
		Sn_5HT[Sn_5HT < 0] = 0.00000001
...
def CBIG_mfm_single_simulation_5HT(parameter, sc_mat, t_epochlong, d_t=0.01):
	'''
	Function used to generate the simulated BOLD signal using mean field
	model and hemodynamic model
	Each parameter set is ussed to simulated one time
	Args:
		parameter:  (N*3+1)*M matrix.
					...
					parameter[2*N+1:3*N+1]: noise amplitude sigma
					parameter[3*N+1:4*N+1]: recurrent strength w_I
					parameter[4*N+1]: W_E_5HT
					parameter[4*N+2]: W_I_5HT
					parameter[4*N+3]: alpha_5HT
		sc_mat:     N*N structural connectivity matrix
		t_epochlong:total simulated time
	Returns:
		bold_d:     simulated BOLD signal
...
def CBIG_mfm_rfMRI_ode_inhibition(y_t, S_I, parameter, sc_mat):
	'''
	This function is to calcualte the derivatives of synaptic gating
	variable S
	Args:
...
def CBIG_mfm_rfMRI_ode_inhibition_5HT(y_t, S_I, parameter, sc_mat, Mn_5HT, Sn_5HT):
	'''
	This function is to calcualte the derivatives of synaptic gating
	variable S
	Args:
		...
		S_I:        N*M matrix represents synaptic gating variable for inhibitory neurons
					N is the number of ROI
					M is the number of candidate parameter sets
		Mn_5HT:     N*M matrix represents neurotransmiter-modulated currents
		Sn_5HT:     N*M matrix represents neurotransmiter concentration
		...
		parameter:  (N*4+1)*M matrix.
					...
					parameter[2*N+1:3*N+1]: noise amplitude sigma
					parameter[3*N+1:4*N+1]: recurrent strength w_I
					parameter[4*N+1]: W_E_%HT
					parameter[4*N+2]: W_I_%HT
					parameter[4*N+3]: alpha_%HT
		sc_mat:     N*N structural connectivity matrix
	Returns:
		dy:         N*M matrix represents derivatives of synaptic gating
		variable S
	...
	I0 = parameter[number_roi:2 * number_roi, :]
	w_I = parameter[number_roi * 3 + 1:number_roi * 4 + 1, :]
	W_E_5HT = parameter[number_roi * 4 + 1, :]
	W_I_5HT = parameter[number_roi * 4 + 2, :]
	alpha_5HT = parameter[number_roi * 4 + 3, :]
	# Parameters for firing rate
	a_E = 310
	b_E = 125
	d_E = 0.16
	...
	beta_5HT = 0.1
	J_5HT = 0.17
	Rn_5HT = csv_matrix_read('D:/hansen_receptors-main/data/PET_parcellated/atlasDA/5HT2a_mdl_hc3_talbot.csv')
	Rn_5HT = torch.from_numpy(Rn_5HT).type(torch.FloatTensor).cuda()
	# Total input x
	x_E = J_E * w_E * y_t + J_E * G.repeat(number_roi, 1) * torch.mm(sc_mat, y_t) + I0 - S_I + W_E_5HT * Rn_5HT * Mn_5HT
	x_I = J_E * w_I * y_t - S_I + 0.7*I0 + W_I_5HT * Rn_5HT * Mn_5HT
	# Population firing rate
	H_E = (a_E * x_E - b_E) / (1 - torch.exp(-d_E * (a_E * x_E - b_E)))
	H_I = (a_I * x_I - b_I) / (1 - torch.exp(-d_I * (a_I * x_I - b_I)))

	# Neurotransmiter dynamics
	dSn_5HT = alpha_5HT * x_E - V_5HT * Sn_5HT / (Km_5HT * Sn_5HT)

	# Electro-neurotransmiter dynamics
	dMn_5HT = 1 / tau_5HT * ( -Mn_5HT + J_5HT / (1 + torch.exp( -beta_5HT * ( torch.log10(Sn_5HT) + 1 ))))
	# Synaptic activity
	dy = -1 / tau_E * y_t + gamma_s * (1 - y_t) * H_E
	dS_I = -1 / tau_I * S_I +  H_I
	return dy, dS_I, dMn_5HT, dSn_5HT
...
def CBIG_FCcorrelation_multi_simulation(emp_fc, bold_d, n_dup):
	...
	W = torch_arctanh(sim_fc_ave)
	H = torch_arctanh(emp_fcm)
...
def CBIG_FCcorrelation_multi_simulation_output(bold_d, n_dup):
	'''
	This function is to calculate the FC correlation cost for multiple
	simulation
	BOLD signal results
	...
	sim_fc = torch.zeros(n_nodes, n_nodes,n_num)
	for i in range(n_num):
		for j in range(n_dup):
			sim_fc[:,:,i] = sim_fc[:,:,i] + torch_corr(bold_d[:, j*n_num+i, :])
		sim_fc[:, :, i] = sim_fc[:, :, i]/n_dup
	return sim_fc
...
def CBIG_FCDKSstat_multi_simulation_WM(emp_ks, bold_d, n_dup):
	'''
	This function is to calculate the FCD KS statistics cost for
	multiple simulation
	BOLD signal results
	...
	for b in range(batch_num):
		bold_temp = bold_d[:, b * sub_num:(b + 1) * sub_num, :]
		bold_tempm = bold_temp.transpose(0, 1).contiguous().view(-1, 405)
		for i in range(0, time_lengh):
			bold_fc = torch_corr(bold_tempm[:, i:i + window_size])
			cor_temp = bold_fc[fc_maskm]
			fc_mat[:, :, i] = torch.transpose(
	...
	fcd_histcumM = fcd_histcum.copy()
	#fcd_histcumM[:, fcd_histcum[-1, :] != emp_ks[-1, 0]] = 0
	fcd_histcumM = fcd_histcumM / fcd_histcumM.max(0)
	emp_ks = emp_ks / emp_ks.max()
...
def CBIG_FCDKSstat_single_simulation_WM(emp_ks, bold_d, n_dup, window_size=83):
	'''
	This function is to calculate the FCD KS statistics cost for single
	simulation
	BOLD signal results
...
def CBIG_combined_cost_train_WM(parameter, n_dup):
	'''
	This function is implemented to calcualted the FC correlation and
	FCD KS
	statistics combined cost for input parameter sets based on training
	...
	emp_fcd = sio.loadmat('F:/fmridymamics_my/WM_atlas_DA/WM_data.mat')
	emp_fcd = np.array(emp_fcd['emp_ks_train'])
	emp_fcd = np.transpose(emp_fcd)

	sc_mat_raw = csv_matrix_read('../../input/Desikan_input/sc_train.csv')
	sc_mat = sc_mat_raw / sc_mat_raw.max() * 0.2
	sc_mat = torch.from_numpy(sc_mat).type(torch.FloatTensor).cuda()

	emp_fc = sio.loadmat('F:/fmridymamics_my/WM_atlas_DA/WM_data.mat')
	emp_fc = np.array(emp_fc['FC_train'])
	emp_fc = torch.from_numpy(emp_fc).type(torch.FloatTensor).cuda()

	# Calculating simualted BOLD signal using MFM
	bold_d = CBIG_mfm_multi_simulation(parameter, sc_mat, 4.86, n_dup)

	# Calculating FC correlation cost
	fc_cost = CBIG_FCcorrelation_multi_simulation(emp_fc, bold_d, n_dup)

	# Calculating FCD KS statistics cost
	fcd_cost = CBIG_FCDKSstat_multi_simulation_WM(emp_fcd, bold_d, n_dup)
...
def CBIG_combined_cost_validation_WM(parameter, n_dup):
	'''
	This function is implemented to calcualted the FC correlation and
	FCD KS
	statistics combined cost for input parameter sets based on
	...
	emp_fcd = sio.loadmat('F:/fmridymamics_my/WM_atlas_DA/WM_data.mat')
	emp_fcd = np.array(emp_fcd['emp_ks_vali'])
	emp_fcd = np.transpose(emp_fcd)
	...
	emp_fc = sio.loadmat('F:/fmridymamics_my/WM_atlas_DA/WM_data.mat')
	emp_fc = np.array(emp_fc['FC_vali'])
	emp_fc = torch.from_numpy(emp_fc).type(torch.FloatTensor).cuda()
