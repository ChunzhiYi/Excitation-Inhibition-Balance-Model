...
def CBIG_mfm_optimization_desikan_main(gpu_index=0, random_seed=1):
    ...
    output_path = 'F:/Thomas Yeo 代码/stable_projects/fMRI_dynamics/Kong2021_pMFM/output/step1_training_results/'
    if not os.path.isdir(output_path):
        os.makedirs(output_path)

    # Initializing input parameters
    highest_order = 1
    N = 4 * (2 * highest_order + 1) + 1
    myelin_data = fc.csv_matrix_read('F:/Thomas Yeo 代码/stable_projects/fMRI_dynamics/Kong2021_pMFM/input/Desikan_input/myelin.csv')
    myelin_data = myelin_data[:, 0]
    gradient_data = fc.csv_matrix_read(
        'F:/Thomas Yeo 代码/stable_projects/fMRI_dynamics/Kong2021_pMFM/input/Desikan_input/rsfc_gradient.csv')
    ...
    search_range[n_node * 2 + 1:n_node * 3 + 1, :] = [0.0005, 0.01]
    search_range[n_node * 3 + 1:dim, :] = [0, 1]
    ...
    start_point_w_I, template_mat = get_init(myelin_data, gradient_data,
                                           highest_order, init_para[n_node * 3 + 1:dim])
    ...
    xmean[2 * (2 * highest_order + 1) + 1:3 * (2 * highest_order + 1) + 1] = start_point_sigma
    xmean[3 * (2 * highest_order + 1) + 1:N] = start_point_w_I
    ...
    D[2 * (2 * highest_order + 1) + 1:3 * (2 * highest_order + 1)+1] = 0.001 / 2
    D[3 * (2 * highest_order + 1) + 1:N] = start_point_w_I[0] / 2
    ...
    while countloop < maxloop:
        ...
        while j < Lambda:
            ...
            input_para[2 * n_node + 1:3*n_node+1, j] = template_mat @ arx[2 * (
                2 * highest_order + 1) + 1:3 * (
                2 * highest_order + 1)+1, j]
