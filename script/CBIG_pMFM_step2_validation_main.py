...
def CBIG_mfm_validation_desikan_main(gpu_index=0):
    ...
    input_path = 'F:/Thomas Yeo 代码/stable_projects/fMRI_dynamics/Kong2021_pMFM/output/step1_training_results/'
    ...
    for i in range(1, 11):
        ...
        para_sigma = template_mat @ x_mass[2 * (2 * highest_order + 1) +
                                           1:3 * (2 * highest_order + 1) + 1, :]
        para_W_I = template_mat @ x_mass[3 * (2 * highest_order + 1) +
                                           1:x_mass.shape[0], :]
